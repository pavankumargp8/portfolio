...
class App {
  ...
  onTouchUp(e) {
    ...
    const dx = endX - this.clickStartX;
    const dy = endY - this.clickStartY;
  ...
  handleCanvasClick(clientX, clientY) {
    if (!this.medias || !this.renderer) return;

    const rect = this.renderer.gl.canvas.getBoundingClientRect();
    const mX = clientX - rect.left;
    const mY = clientY - rect.top;

    const viewportX = (mX / rect.width - 0.5) * this.viewport.width;
    const viewportY = (0.5 - mY / rect.height) * this.viewport.height;

    for (const media of this.medias) {
      const px = media.plane.position.x;
      const py = media.plane.position.y;
      const sx = media.plane.scale.x / 2;
      const sy = media.plane.scale.y / 2;
